...
from djangorag.lightrag_app.core import LightRAGCore, QueryParam
            ...
            try:
                ...
                if include_sources and result.sources:
                    ...
                    for i, source in enumerate(result.sources, 1):
                        self.stdout.write(f'{i}. [{source["type"].upper()}] {source.get("name", source.get("id", ""))}')
                        if 'content' in source:
                            self.stdout.write(f'   {source["content"][:200]}...')
                        if 'document_title' in source:
                            self.stdout.write(f'   Document: {source["document_title"]}')
                ...
                if include_context:
                    ...
                    self.stdout.write(f'Total Context Tokens: {context.get("total_tokens", 0)}')
            finally:
                core.close()
